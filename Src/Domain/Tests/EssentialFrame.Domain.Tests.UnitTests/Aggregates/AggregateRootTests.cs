...
public sealed class AggregateRootTests
{
    ...
    public void Rehydrate_WhenHistoryIsValid_ShouldRestoreCorrectAggregate()
    {
        ...
        ChangeTitleDomainEvent changeTitleEvent =
            new(aggregateIdentifier, identityContext, aggregateVersion + 1, title);

        ChangeDescriptionDomainEvent changeDescription =
            new(aggregateIdentifier, identityContext, aggregateVersion + 2, description);

        ChangeExpirationDateDomainEvent changeExpirationEvent =
            new(aggregateIdentifier, identityContext, aggregateVersion + 3, expiration);

        AddImagesDomainEvent addImagesEvent = new(aggregateIdentifier, identityContext, aggregateVersion + 4, images);

        ChangeImageNameDomainEvent changeImageNameEvent =
            new(aggregateIdentifier, identityContext, aggregateVersion + 5, imageId, imageName);
        ...
        MutateAggregateState_WithOutdatedExpirationDate_ShouldThrowExpiredPostCannotBeUpdatedRuleValidationException()
    {
        ...
        // Act
        Action changeTitleAction = () => aggregate.ChangeTitle(title, identityContext);
        Action changeDescriptionAction = () => aggregate.ChangeDescription(description, identityContext);
        Action extendExpirationAction = () => aggregate.ExtendExpirationDate(expiration, identityContext);
        Action changeImageNameAction = () => aggregate.ChangeImageName(imageId, imageName, identityContext);
    ...
    public async Task ExtendExpirationDate_WhenAggregateIsOutdated_ShouldBePossibleToExtendExpirationDate()
    {
        ...
        // Act
        Action extendExpirationAction = () => aggregate.ExtendExpirationDate(newExpiration, identityContext);
    ...
    public void AddNewImage_WithNotUniqueName_ShouldThrowImageNameMustBeUniqueWithInPostRuleValidationException()
    {
        ...
        Action addImagesAction = () =>
            aggregate.AddImages(
                new HashSet<Image> { Image.Create(_faker.Random.Guid(), imageName, _faker.Random.Bytes(200)) },
                identityContext);
    ...
    public void ChangeImageName_WithNotUniqueName_ShouldThrowImageNameMustBeUniqueWithInPostValidationException()
    {
        ...
        Action addImagesAction = () => aggregate.ChangeImageName(imageId, duplicateImageName, identityContext);
