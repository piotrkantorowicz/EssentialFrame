...
{
    public CommandData(ICommand command)
    {
        CommandIdentifier = command.CommandIdentifier;

        Validate(command,
                 command.GetTypeFullName(),
    ...
    public virtual void Start()
    {
        ValidateCommandSendStatuses(new[]
                                    {
                                        CommandSendingStatuses.Started,
                                        CommandExecutionStatuses.WaitingForExecution
                                    });
    ...
    public virtual void Schedule(DateTimeOffset? at)
    {
        ValidateCommandSendStatuses(new[]
                                    {
                                        CommandSendingStatuses.Scheduled,
                                        CommandExecutionStatuses.WaitingForExecution
                                    });
    ...
    public virtual void Cancel()
    {
        ValidateCommandSendStatuses(new[]
                                    {
                                        CommandSendingStatuses.Cancelled,
                                        CommandExecutionStatuses.ExecutionCancelled
                                    });
    ...
    public virtual void Complete(bool success)
    {
        ValidateCommandSendStatuses(new[]
                                    {
                                        CommandSendingStatuses.Completed,
    ...
    {
        foreach (var commandSendStatus in commandStatuses)
        {
            if (commandSendStatus?.Length is <= 3 or >= 30)
            {
                throw new
                    OverflowException($"Command status have to be longer or equal than 3 characters and shorter or equal than 30. Value: {commandSendStatus}");
            }
        }
    }
                                 ...
                                 string commandClass)
    {
        ...
        {
            throw new
                OverflowException($"Command type have to be longer or equal than 3 characters and shorter or equal than 100. Value: {commandType}");
        }

        if (commandClass?.Length is <= 3 or >= 250)
        {
            throw new
                OverflowException($"Command class have to be longer or equal than 3 characters and shorter or equal than 250. Value: {commandClass}");
        }
    }
}
