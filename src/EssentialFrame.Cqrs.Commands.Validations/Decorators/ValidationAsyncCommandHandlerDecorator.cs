    ...
    where TCommand : class, ICommand
{
    ...
    internal ValidationAsyncCommandHandlerDecorator(IList<CommandAbstractValidator<TCommand>> validators,
                                                    IAsyncCommandHandler<TCommand> decorated,
                                                    ILogger<ValidationAsyncCommandHandlerDecorator<TCommand>> logger)
    {
    ...
    public async Task<ICommandResult> HandleAsync(TCommand command, CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(LoggingUtils.UnexpectedException,
                             ex,
                             "Unexpected exception occurred when validating command: {Command}",
                             command.GetTypeFullName());

            return
                CommandResult
                    .Fail(new
                              UnexpectedError($"Unexpected exception occurred while validating command: {command.GetTypeFullName()}",
                                              ex));
        }

        _logger.LogDebug(LoggingUtils.ValidationSuccess,
                         "Validation passed for {Command}",
                         command.GetTypeFullName());
